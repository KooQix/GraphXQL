...
import dev.kooqix.avro.DataFileStream2.DataBlock;

/**
 * DataFileWriter needs to be Serializable, so this class extends DataFileWriter
 * and implements Serializable
 */
public class DataFileWriter2<D> implements Closeable, Flushable, Serializable {
	...
	public DataFileWriter2(DatumWriter<D> dout) {
		this.dout = dout;
	}
	 ...
	 * have begun.
	 */
	public DataFileWriter2<D> setCodec(CodecFactory2 c) {
		assertNotOpen();
	 ...
	 */
	public DataFileWriter2<D> setSyncInterval(int syncInterval) {
	...
	public DataFileWriter2<D> create(Schema schema, File file) throws IOException {
	...
	/** Open a new file for data matching a schema with a random sync. */
	public DataFileWriter2<D> create(Schema schema, OutputStream outs) throws IOException {
		return create(schema, outs, null);
	}

	/** Open a new file for data matching a schema with an explicit sync. */
	public DataFileWriter2<D> create(Schema schema, OutputStream outs, byte[] sync) throws IOException {
		assertNotOpen();
	...
	public DataFileWriter2<D> appendTo(File file) throws IOException {
	 ...
	 */
	public DataFileWriter2<D> appendTo(SeekableInput in, OutputStream out) throws IOException {
		assertNotOpen();
		DataFileReader2<D> reader = new DataFileReader2<>(in, new GenericDatumReader<>());
		...
		if (codecBytes != null) {
			...
			this.codec = CodecFactory2.fromString(strCodec).createInstance();
		} else {
			this.codec = CodecFactory2.nullCodec().createInstance();
		}
	...
	private DataFileWriter2<D> setMetaInternal(String key, byte[] value) {
		assertNotOpen();
		meta.put(key, value);
		return this;
	}

	private DataFileWriter2<D> setMetaInternal(String key, String value) {
		return setMetaInternal(key, value.getBytes(UTF_8));
	}

	/** Set a metadata property. */
	public DataFileWriter2<D> setMeta(String key, byte[] value) {
	...
	/** Set a metadata property. */
	public DataFileWriter2<D> setMeta(String key, String value) {
		return setMeta(key, value.getBytes(UTF_8));
	}

	/** Set a metadata property. */
	public DataFileWriter2<D> setMeta(String key, long value) {
