 ...
 * @see DataFileWriter
 */
public class DataFileReader2<D> extends DataFileStream<D> implements FileReader<D> {
	...
	public static <D> FileReader<D> openReader(SeekableInput in, DatumReader<D> reader) throws IOException {
		...
		if (Arrays.equals(MAGIC, magic)) // current format
			return new DataFileReader2<>(in, reader, magic);
		if (Arrays.equals(DataFileReader12.MAGIC, magic)) // 1.2 format
			return new DataFileReader2<>(in, reader);
	 ...
	 */
	public static <D> DataFileReader2<D> openReader(SeekableInput in, DatumReader<D> reader, Header header,
			boolean sync)
			throws IOException {
		DataFileReader2<D> dreader = new DataFileReader2<>(in, reader, header);
	 ...
	 * <pre/>
	 */
	public DataFileReader2(File file, DatumReader<D> reader) throws IOException {
	 ...
	 * <pre/>
	 */
	public DataFileReader2(SeekableInput sin, DatumReader<D> reader) throws IOException {
		this(sin, reader, false, null);
	}

	private DataFileReader2(SeekableInput sin, DatumReader<D> reader, byte[] magic) throws IOException {
		this(sin, reader, false, magic);
	}

	/** Construct a reader for a file. Please close resource files yourself. */
	protected DataFileReader2(SeekableInput sin, DatumReader<D> reader, boolean closeOnError, byte[] magic)
			throws IOException {
		super(reader);
		try {
	 ...
	 */
	protected DataFileReader2(SeekableInput sin, DatumReader<D> reader, Header header) throws IOException {
		super(reader);
		this.sin = new SeekableInputStream(sin);
		initialize(header);
	}
